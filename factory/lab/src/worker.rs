...
async fn lab_worker_one(log: &Logger, c: &Central) -> Result<()> {
    ...
    if !supported_targets.is_empty() {
        ...
        if let Some(lease) = c
            .client
            .factory_lease()
            .body(FactoryWhatsNext { supported_targets })
            .send()
            .await?
            .into_inner()
            .lease
            ...
            if let Some(t) = c.config.target.get(&lease.target) {
                if ready_hosts.contains(&t.nodename) {
                    ...
                    c.client
                        ...
                        .worker(w.id.to_string())
                        .body(FactoryWorkerAssociate { private: i.id() })
                        .send()
                        .await?;
                    info!(
                        log,
