...
async fn factory_task_one(log: &Logger, c: &Arc<Central>) -> Result<()> {
    ...
    for i in instances {
        ...
        let id = i.id();
        let t = c.config.for_instance(&id)?;
        let w = c
            .client
            .factory_worker_get()
            .worker(&i.worker)
    ...
    /*
     * We currently only support one job on a particular physical host for this
     * factory.
     */
    let active = c.db.instances_active()?;
    if !active.is_empty() {
        /*
    ...
    let t = c.config.for_instance(&instance_id)?;
