...
async fn process_changes(
    ...
    while let Some(c) = changes.try_next().await? {
        ...
        /*
         * XXX Use change messages to assign a publish date to revisions.
         */
        let mut dates: HashMap<u32, String> = Default::default();
        let mut msgs = client.change_messages(&c.change_id);
        while let Some(m) = msgs.try_next().await? {
            if let Some(tag) = &m.tag {
                ...
                {
                    if let Some(num) = m._revision_number {
                        dates.insert(num, m.date.to_string());
                    }
                }
            }
            //info!(log, "MESSAGE: {:?}", m);
        ...
        for rev in full.revisions() {
            ...
            let date = if let Some(dt) = dates.get(&rev._number) {
                dt.to_string()
            } else {
                bail!("revision {}/{} has no date?", c.change_id, rev._number);
            };
