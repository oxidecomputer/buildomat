...
fn files_to_cache(metadata: &CargoMetadata) -> Result<Vec<PathBuf>> {
    /*
     * Caching the build artifacts of path dependencies has diminishing results,
     ...
     * artifacts of third-party dependencies.
     */
...
impl FilesToCache {
    /*
     * "Profile directories" are the directories containing the build artifacts
    ...
    fn collect_directory_with_packages(&mut self, path: &Path) -> Result<()> {
        for entry in path.read_dir()? {
            ...
            let Some((name, _hash)) = file_name
                .to_str()
                .ok_or_else(|| anyhow!("non-utf-8 name"))?
                .rsplit_once('-')
            else {
                continue;
            };
    ...
    fn collect_directory_with_targets(&mut self, path: &Path) -> Result<()> {
        for entry in path.read_dir()? {
             ...
             */
            if self.cacheable_targets.contains(name)
                || self
                    .cacheable_targets
                    .contains(name.strip_prefix("lib").unwrap_or(name))
            {
                self.collect_recursive(&entry.path())?;
            }
        }
...
fn cache_key(metadata: &CargoMetadata) -> Result<String> {
    ...
    Ok(format!("rust-{rustc_version}-{lockfile_digest}"))
}
