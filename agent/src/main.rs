...
async fn cmd_run(mut l: Level<()>) -> Result<()> {
    ...
    let mut urx: Option<mpsc::Receiver<upload::Activity>> = None;
    let mut drx: Option<mpsc::Receiver<download::Activity>> = None;
    ...
    //let x = urx.as_mut().unwrap().recv().await;

    loop {
        let what = tokio::select! {
            _ = pingfreq.tick() => What::Ping,
            ua = urx.as_mut().unwrap().recv(), if urx.is_some() => {
                if let Some(ua) = ua {
                    What::Upload(ua)
                } else {
                    /*
            ...
            da = drx.as_mut().unwrap().recv(), if drx.is_some() => {
                if let Some(da) = da {
                    What::Download(da)
                } else {
                    /*
        ...
        match what {
            /*
             * First, take care of submitting our current status to the server.
             * The ping will contain instructions if we need to start executing
             * a job, etc.
             */
            What::Ping => match cw.client.worker_ping().await {
                Ok(p) => {
                    if p.poweroff {
            ...
            What::Download(a) => {}
            What::Upload(a) => {
                assert!(matches!(stage, Stage::Upload));

                match a {
                    upload::Activity::Scanned(count) => {
                        cw.append_msg(&format!("found {} output files", count))
                            .await;
                    }
                    upload::Activity::Uploading(p, sz) => {
            ...
            What::Download(a) => {
                ...
                match a {
                    ...
                    download::Activity::Downloaded(p) => {
                        cw.append_msg(&format!(
                            "downloaded input: {}",
                            p.display(),
                        ))
