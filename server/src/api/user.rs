...
pub(crate) async fn job_watch(
    ...
    if let Some(mut rx) = c.db.job_subscribe(&j) {
        ...
        /*
         * Record the last sequence number and job state generation number we
         * have seen, starting with the first number we get from the watch:
         */
        let (mut seq, mut gen) = {
            let rx = rx.borrow();
        ...
        /*
         * Now that we have a subscription, generate the public description of
         * the job state.
         */
        ...
        tokio::task::spawn(async move {
            ...
            loop {
                /*
                 * Before we check for job records, render the job state for the
                 * client.
                 */
                ...
                let output = if new_state != previous_state && events.is_empty()
                {
                    /*
                     * Just send on the updated state notification.
                     */
                    let out = format!("event: state\ndata: {new_state}\n\n");
                    ...
                    match act {
                        ...
                        Act::Watch(eof) => {
                            ...
                            if gen != jn.gen {
                                /*
                                 * There has been a job state change.  Go back
                    ...
                    if new_state != previous_state {
                        /*
                         * Send the updated state notification first:
                         */
                        out += &format!("event: state\ndata: {new_state}\n\n");
                        previous_state = new_state;
                    }
                    ...
                    for ev in events {
                        ...
                        out += "event: job\ndata: ";
                        out += &serde_json::to_string(&JobEvent::from(ev))
                            .or_500()?;
                        out += "\n\n";
                    }
                    out
